...
├── LICENSE             # MIT
├── scripts/            # 13 个确定性 CLI + check-deps.sh 前置自检，扁平单层
├── judges/             # 7 份 LLM 评审 rubric（md）
└── tests/              # 169 个测试，扁平单层，无 conftest
docs/adr/               # 13 条架构决策（0001–0013）
...
| 改 8 步流程本身 | `skill-evaluator/SKILL.md` |
...
python -m pytest tests                # 全量：169 个测试
...
python scripts/evalset_check.py evalsets/<NAME>/v1 --skill <被测skill目录>
