...
多个指标共用同一次沙箱运行——6 类运行场景按档位（T0–T4，默认 T2）增量编组，见 `skill-evaluator/reference.md`（§ 运行场景）与 ADR-0014。
...
└── tests/              # 229 个测试，全部走 subprocess seam
docs/adr/               # 16 条架构决策记录
```
...
```bash
cd skill-evaluator
python -m pytest tests        # 229 个测试全绿
```
