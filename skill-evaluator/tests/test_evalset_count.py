"""evalset_count.py 断链修复前先立契约：条数闸门的计数口径、配置优先级、坏文件不计。

Seam: python scripts/evalset_count.py <evalsets/<name>/vN> [--out <file>] [--min N]
...
SCRIPT = Path(__file__).parent.parent / "scripts" / "evalset_count.py"
...
def run_check(ev: Path, *args, env=None):
    ...
    assert r.returncode == 0, f"evalset_count.py failed: {r.stderr}"
