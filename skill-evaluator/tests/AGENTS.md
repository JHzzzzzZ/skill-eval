...
15 个测试文件 / 169 个 test，全部走 subprocess seam：把脚本当 CLI 跑，不 import 被测脚本。

## WHERE TO LOOK

| 测试文件 | 覆盖 | test 数 |
|---|---|---|
| `test_static_check.py` | `scripts/static_check.py`（含五组规则） | 34 |
| `test_report.py` | `report.py` 19-key 契约基线 | 22 |
| `test_report3.py` | `report.py` 元数据/指纹批次（ADR-0010/0011） | 21 |
...
| `test_evalset_check.py` | `scripts/evalset_check.py` | 11 |
...
`scripts/` 13 个 Python 脚本全覆盖，无缺口。`report.py` 是唯一拆成 3 个文件覆盖的脚本。`check-deps.sh` 无 Python 测试（前置自检靠实跑验证）。
