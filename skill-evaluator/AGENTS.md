...
| 改扫描口径 / 五组规则表 | `docs/adr/0012`（口径）、`docs/adr/0013`（五组分级）+ `scripts/static_check.py:45-123` |
| 改 fallback 行为 | `reference.md § Fallback` 表 |

## PIPELINE

SKILL.md 的 8 步。脚本之间**没有 subprocess 调用**，串联只靠 `evalsets/<name>/results/<version>/` 下的产物文件：
...
4. 评测集冻结；冻结前 `evalset_check.py` → `evalset.json`（#1 可信度）
